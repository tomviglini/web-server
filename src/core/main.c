...
enum http_status_code { HTTP_200, HTTP_206, HTTP_404, HTTP_500 };
...
struct HTTP_STATUS_CODE_LINE http_status[] = {
    ...
    [HTTP_206] = {.len = sizeof("HTTP/1.1 206 Partial Content\r\n") - 1,
                  .ptr = "HTTP/1.1 206 Partial Content\r\n"},
    [HTTP_404] = {.len = sizeof("HTTP/1/1 404 Not Found\r\n") - 1,
                  .ptr = "HTTP/1/1 404 Not Found\r\n"},
    [HTTP_500] = {.len = sizeof("HTTP/1/1 500 Internal Server Error\r\n") - 1,
                  .ptr = "HTTP/1.1 500 Internal Server Error\r\n"}};
...
__attribute__((always_inline)) static int load_shared_object(void *handle, char *function_name,
                                                             struct PATH *record) {
...
void load(struct SERVER *server, struct RECORD *record, void *handle, char *external,
          char *internal) {
...
void parse(struct SERVER *server) {
    // listen_create_old2(server, "0.0.0.0", "80", "example2.com");
...
enum WORKER_STATUS { WORKER_LOCK = 1, WORKER_BUSY = 2, WORKER_PENDING = 4 };
    ...
    while (1) {
        ...
        if (busy == 1) {
            ...
            if (pending == 1) {
                ...
                /* if the connection is close or the http version is not 1.1 and the connection is
                 * not set close */
                if (System.Request.Header.Connection == HTTP_CONNECTION_CLOSE ||
                    (System.Request.Version != HTTP_VERSION_1_1 &&
                     System.Request.Header.Connection == HTTP_CONNECTION_NULL)) {
                    close(queue_value);
                }
            ...
            if (server->connection.record[queue_value].pending == 1) {
                ...
                /* update the timeout */
                server->connection.record[queue_value].timeout =
                    time(NULL) + CONNECTION_KEEP_ALIVE_TIMEOUT;
        ...
        /* search the host */
        lget(&server->listener.record[server->connection.record[queue_value].listener].host,
             &record1, System.Request.Header.Host);
        ...
        if (record2 != NULL) {
            ...
            switch (path->path_type) {
                case PATH_TYPE_DYNAMIC:
                    ...
                    // *)record2->val)->val)->path)->function(1, argv);
                    ...
                    if (System.Response.Header.Content_Type == NULL) {
                        string_printf(&System.Response.Header3, "Content-Type: text/html\r\n");
                    }

                    string_printf(&System.Response.Header3,
                                  "Content-Length: %u\r\n"
                                  "Connection: Keep-Alive\r\n"
                                  "Server: TheNext\r\n"
                                  "\r\n",
                ...
                case PATH_TYPE_STATIC:
                    ...
                    /*if(
                            ...
                            ((int *)((struct STATIC *)((struct PATH *)((struct RECORD
                    *)record2->val)->val)->path)->crc)[0] == ((int *)
                    System.Request.Header.If_None_Match)[0] &&
                            ((int *)((struct STATIC *)((struct PATH *)((struct RECORD
                    *)record2->val)->val)->path)->crc)[1] == ((int *)
                    System.Request.Header.If_None_Match)[1] &&
                            ((int *)((struct STATIC *)((struct PATH *)((struct RECORD
                    *)record2->val)->val)->path)->crc)[2] == ((int *)
                    System.Request.Header.If_None_Match)[2] &&
                            ((int *)((struct STATIC *)((struct PATH *)((struct RECORD
                    *)record2->val)->val)->path)->crc)[3] == ((int *)
                    System.Request.Header.If_None_Match)[3] ) {
                    ...
                    if (System.Request.Method == HTTP_METHOD_HEAD) {
                        /*iov_num = 1;
                        iov[0].iov_base = ((struct STATIC *)((struct PATH *)((struct RECORD
                        *)record2->val)->val)->path)->ptr; iov[0].iov_len 	= ((struct STATIC
                        *)((struct PATH *)((struct RECORD
                        *)record2->val)->val)->path)->header_size;*/
                        ...
                        iov[0].iov_base = ((struct STATIC *)((struct PATH *)((struct RECORD
                        *)record2->val)->val)->path)->ptr; iov[0].iov_len 	= ((struct STATIC
                        *)((struct PATH *)((struct RECORD *)record2->val)->val)->path)->size;*/
                ...
                case PATH_TYPE_ERROR:
                    ...
                    if (System.Request.Method == HTTP_METHOD_HEAD) {
                        ...
                        iov[0].iov_base = ((struct ERROR *)((struct PATH *)((struct RECORD
                        *)record2->val)->val)->path)->ptr; iov[0].iov_len 	= ((struct ERROR
                        *)((struct PATH *)((struct RECORD
                        *)record2->val)->val)->path)->header_size;*/
                    } else {
                        /*iov_num = 1;
        ...
        for (i = 0; i < epoll_fd_ready; i++) {
            if (events[i].events & EPOLLERR || events[i].events & EPOLLHUP ||
                ...
                if (server->connection.record[events[i].data.fd].busy == 1) {
                    ...
                    server->worker.record[server->connection.record[events[i].data.fd].worker]
                        .close_while_busy = 1;
                }
                ...
                continue;
            } else if (server->listener.size > events[i].data.fd &&
                       server->listener.record[events[i].data.fd].active == 1) {
                while (1) {
                    return_value =
                        _socket_accept(&events[i].data.fd, &accepted_fd, &address, &address_len);
                    if (return_value == 1) {
                        break;
                    }
