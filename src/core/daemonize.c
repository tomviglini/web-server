...
int daemonize(int argc, char *argv[], struct SERVER *server) {
    ...
    if (argc > 1) {
        if (result = strcmp(argv[1], "-start") == 0) {
            ...
            printf("\n%s (%s %s) %s %s\n\n", "Web-Server-Alpha-Build", __DATE__, __TIME__,
                   "https://twitter.com/tomviglini", "https://github.com/tomviglini/web-server");
            ...
            printf(
                ...
                "%-15s %s\n"
                "\n",
                "-start", "start the daemon", "-stop", "stop the daemon", "--version",
                "return the version number and build date", "--help", "display this information");
