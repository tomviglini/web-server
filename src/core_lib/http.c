...
__attribute__((always_inline)) int http_parse(struct HTTP_REQUEST *http, char *request,
                                              char *buffer, register int len, int *error) {
    /* ------------------------------------------------------------------------------------------ */
    register int i;
    register int n;
    int skip;
        ...
        if (len >= i + sizeof(int)) {
            switch (*((unsigned int *)&request[i])) {
                case _HOST_1:
                    ...
                    if(request[i+0] == '\r') { write(1, "<CR>", sizeof("<CR>") - 1); } else
                    if(request[i+0] == '\n') { write(1, "<LF>", sizeof("<LF>") - 1); } else {
                    write(1, &request[i+0], 1); } if(request[i+1] == '\r') { write(1, "<CR>",
                    sizeof("<CR>") - 1); } else if(request[i+1] == '\n') { write(1, "<LF>",
                    sizeof("<LF>") - 1); } else { write(1, &request[i+1], 1); } if(request[i+2] ==
                    '\r') { write(1, "<CR>", sizeof("<CR>") - 1); } else if(request[i+2] == '\n') {
                    write(1, "<LF>", sizeof("<LF>") - 1); } else { write(1, &request[i+2], 1); }
                    if(request[i+3] == '\r') { write(1, "<CR>", sizeof("<CR>") - 1); } else
                    if(request[i+3] == '\n') { write(1, "<LF>", sizeof("<LF>") - 1); } else {
                    write(1, &request[i+3], 1); } write(1, "\n", sizeof("\n") - 1); sleep(1);*/
                ...
                case _CONNECTION_1:
                    ...
                    if (*((unsigned int *)&request[i + 4]) == _CONNECTION_2 &&
                        *((unsigned int *)&request[i + 8]) == _CONNECTION_3) {
                        ...
                        // LAZY CHECK TO BE REVISED CONSIDER THE HTTP/1.0/HTTP/1.1 'K-k' && 'A-a'
                        // VARIABLE
                        if (request[i] == 'C') {
                        ...
                        switch (*((unsigned int *)&request[i + 8])) {
                            ...
                            case _CONTENT_TYPE_1:
                                ...
                                if (*((unsigned short *)&request[i + 12]) == _CONTENT_TYPE_2) {
                                    ...
                                    if (*((unsigned int *)&request[i]) ==
                                            _APPLICATION_URL_ENCODED_1 &&
                                        *((unsigned int *)&request[i + 4]) ==
                                            _APPLICATION_URL_ENCODED_2 &&
                                        *((unsigned int *)&request[i + 8]) ==
                                            _APPLICATION_URL_ENCODED_3 &&
                                        *((unsigned int *)&request[i + 12]) ==
                                            _APPLICATION_URL_ENCODED_4 &&
                                        *((unsigned int *)&request[i + 16]) ==
                                            _APPLICATION_URL_ENCODED_5 &&
                                        *((unsigned int *)&request[i + 20]) ==
                                            _APPLICATION_URL_ENCODED_6 &&
                                        *((unsigned int *)&request[i + 24]) ==
                                            _APPLICATION_URL_ENCODED_7 &&
                                        *((unsigned int *)&request[i + 28]) ==
                                            _APPLICATION_URL_ENCODED_8 &&
                                        *((unsigned int *)&request[i + 32]) ==
                                            _APPLICATION_URL_ENCODED_9) {
...
__attribute__((always_inline)) void http_print_buffer(char *buffer, int size) {
    int i;
...
__attribute__((always_inline)) void http_print_env(struct HTTP_REQUEST *Request) {
    printf("METHOD: ");
    ...
    if (Request->Header.Content_Type != HTTP_CONTENT_TYPE_NULL) {
        ...
        switch (Request->Header.Content_Type) {
            case HTTP_CONTENT_TYPE_APPLICATION_URL_ENCODED:
                printf("APPLICATION_URL_ENCODED");
                break;
        }
        printf("\n");
    }
...
__attribute__((always_inline)) void http_simulate(struct HTTP_REQUEST *http_request, char *buffer) {
